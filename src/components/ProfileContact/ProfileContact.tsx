...
const ProfileContact: FC<IProps> = ({ person }) => {
  ...
  return (
    <div>
      <div className={`${style.profileTitle} ${style.whiteText}`}>
        Contact me
      </div>
      <div
        className={`${style.profileText} ${style.whiteText} ${style.profileContactContainer}`}
      >
        ...
        {facebook && (
          <SocialWebLink link={facebook} icon={faFacebook} title={"facebook"} />
        )}
        {telegram && (
          <SocialWebLink link={telegram} icon={faTelegram} title={"telegram"} />
        )}
      </div>
    </div>
  );
